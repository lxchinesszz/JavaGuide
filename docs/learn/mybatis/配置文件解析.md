...
```java 
    ...
    public void propertyParser() {
        ...
        // 变量中有就从变量中获取 com.mysql.cj.jdbc.Driver
        System.out.println(PropertyParser.parse("${datasource.driver-class-name}", variables));
...
在Mybatis只使用到的反射地方蛮多的，那么都知道反射是相对比较耗时间，那么我们来看Mybatis是如何利用反射工厂来提高反射的性能的?
